...
/*   Updated: 2017/12/20 12:10:21 by skamoza          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	wolf_step(t_map *map, t_player *player, double speed)
{
	...
	if (new_pos.x >= 0 && new_pos.x < map->map.w &&  new_pos.y >= 0 &&
	new_pos.y < map->map.h && !(map->map.data[(int)(player->pos.y *
	(map->map.size_line >> 2) + new_pos.x)]))
		player->pos.x = new_pos.x;
	if (new_pos.x >= 0 && new_pos.x < map->map.w &&  new_pos.y >= 0 &&
	new_pos.y < map->map.h && !(map->map.data[(int)(new_pos.y *
